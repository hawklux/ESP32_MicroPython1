...
# 액션 목표값을 처리하는 로직을 구현
class ActionServer5(Node):
    # 액션 서버를 생성하는 노드
    def __init__(self):
        super().__init__('act_srvr5_thread_node')
        ...
        self.action_server = ActionServer(self, Action1, 'act_srvr5_thread', self.execute_callback)
        
        # srvr3 추가: 파라미터 설명자 생성
        # 주: floating_point_range는 ParameterDescriptor.msg 파일에 정의된 속성명이므로 다른 이름 사용 불가
        quantile_time_desc = ParameterDescriptor(
            description='터틀 움직임에 대한 Quantile time', 
            floating_point_range=[FloatingPointRange(
                from_value=0.0, to_value=1.0, step=0.01
            )]
        )
        # quantile_time_desc = ParameterDescriptor(
        #     type=ParameterDescriptor.PARAMETER_DOUBLE,
        #     description='Quantile time for turtle movement',
        #     floating_point_range=[FloatingPointRange(min_value=0.0, max_value=1.0)]
        # )

        # srvr3 수정: 파라미터 선언
        self.declare_parameter('quantile_time', 0.75, quantile_time_desc)
